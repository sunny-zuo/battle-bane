package {
	...
	public class GameHandler extends MovieClip {
		...
		public static var joystickLeft: Joystick;
		public static var joystickLeftID: int;
		public static var joystickRight: Joystick;
		public static var joystickRightID: int;
		...
		private function addedToStage(event: Event): void {
			...
			stage.addEventListener(TouchEvent.TOUCH_BEGIN, createJoystick);
			stage.addEventListener(TouchEvent.TOUCH_END, removeJoystick);
		...
		private function update(event: TimerEvent): void {
			...
			if (Boolean(this.getChildByName(username)) == true) {
				...
				if (joystickRight != null) {
					if (Math.abs(joystickRight.xOutput()) > 0.2 || Math.abs(joystickRight.yOutput()) > 0.2) {
						degrees = joystickRight.angleOutput();
		...
		private function updatePlayerMovement() {
			if (Boolean(this.getChildByName(username)) == true && joystickLeft != null) {
				//if the player exists, select it
				var target2: MovieClip = this.getChildByName(username) as MovieClip;
				var xPos = target2.x;
				var yPos = target2.y;
				if (joystickLeft.yOutput() > 0.2) {
					target2.y = charInfo[4] + (speed * joystickLeft.yOutput()); //move player location temporarily to make sure no collision will happen after proposed move
					if (!SkyCollisionDetection.bitmapHitTest(target2, walls)) {
						charInfo[4] += speed * joystickLeft.yOutput(); //change location to be broadcast						}
						target2.y = yPos //changes player location back to where it was
					}
				}

				if (joystickLeft.xOutput() < -0.2) {
					target2.x = charInfo[3] + (speed * joystickLeft.xOutput());
					target2.y = charInfo[4];
					if (!SkyCollisionDetection.bitmapHitTest(target2, walls)) {
						charInfo[3] += speed * joystickLeft.xOutput();
					}
					target2.x = xPos;
					target2.y = yPos;
				}
				if (joystickLeft.yOutput() < -0.2) {
					target2.y = charInfo[4] + (speed * joystickLeft.yOutput());
					target2.x = charInfo[3];
					if (!SkyCollisionDetection.bitmapHitTest(target2, walls)) {
						charInfo[4] += speed * joystickLeft.yOutput();
					}
					target2.x = xPos;
					target2.y = yPos;
				}
		...
		private function deconstruct(killerName: String): void {
			stage.removeEventListener(TouchEvent.TOUCH_BEGIN, createJoystick);

			if (joystickLeft != null) {
				joystickLeft.cleanup();
				joystickLeft = null;
			}
			if (joystickRight != null) {
				joystickRight.cleanup();
				joystickRight = null;
			}
			...
			websocket = null;
			application.addEventListener(Event.DEACTIVATE, appDeactivated);
		...
		private function createJoystick(event: TouchEvent): void {
			if (joystickLeft == null && event.stageX < (stage.stageWidth / 4)) {
				joystickLeftID = event.touchPointID;
				joystickLeft = new Joystick(); //create joystick
				joystickLeft.x = event.stageX;
				joystickLeft.y = event.stageY;
				//place joystick where mouse is
				stage.addChild(joystickLeft);
				//add joystick
			}

			if (joystickRight == null && event.stageX > (stage.stageWidth * 3 / 4)) {
				joystickRightID = event.touchPointID;
				joystickRight = new Joystick(); //create joystick
				joystickRight.x = event.stageX;
				joystickRight.y = event.stageY;
				//place joystick where mouse is
				stage.addChild(joystickRight);
				//add joystick
			}
		}

		private function removeJoystick(event: TouchEvent): void {
			if (event.touchPointID == joystickRightID) {
				fireBullet();
				joystickRight.cleanup();
				joystickRight = null;
			}
			if (event.touchPointID == joystickLeftID) {
				joystickLeft.cleanup();
				joystickLeft = null;
			}
		}
		...
		private function appDeactivated(event: Event): void {
			websocket.sendUTF(serverName + ",deathData," + username + "," + lastHitBy);
			hasConnected = false;
			...
			charInfo = null; //remove charInfo and websocket
			websocket = null;
			application.removeEventListener(Event.DEACTIVATE, appDeactivated);
